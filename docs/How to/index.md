...
DJSUPERADMIN = {
    "INPLACE_EDIT": True,
    "IMAGE_GALLERY_URL": "/api/media/",  # your media list endpoint
    # "IMAGE_UPLOAD_URL": "/api/media/upload/",  # optional, for uploads
}
```
...
This is meant to plug into a CMS media library: point `IMAGE_GALLERY_URL` at its
media API and editors can drop existing media straight into the content. (See the
`example/` project for a working demo endpoint.)
